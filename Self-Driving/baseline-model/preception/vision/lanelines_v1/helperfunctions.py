...
def abs_sobel_thresh(img, orient='x', sobel_kernel=3, thresh=(0, 255)):
    ...
    # Step 3: Abs value of Derivative
    abs_derivative = np.absolute(derivative)

    # Step 4: Scale to 8-bit (0 - 255)
...
def dir_threshold(image, sobel_kernel=3, thresh=(0, np.pi / 2)):
    ...
    # Step 3: Abs value of sobelX sobelY
    abs_sobelx = np.absolute(sobelx)
    abs_sobely = np.absolute(sobely)

    # Step 4: Get direction of the gradient
...
def apply_thresholds(image, ksize=3):
    # Step 1:  Apply each of the thresholding functions
